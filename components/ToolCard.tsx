...
export default function ToolCard({tool,onOpen}:{tool:Tool;onOpen:(t:Tool)=>void}){
 return <motion.button onClick={()=>onOpen(tool)} whileHover={{y:-5}} whileTap={{scale:.985}} className="text-right w-full glass rounded-3xl p-5 transition hover:border-gold/30 hover:shadow-gold-glow group">
   <div className="flex items-start justify-between gap-4"><div className="flex items-center gap-3"><div className="h-12 w-12 rounded-2xl grid place-items-center bg-gradient-to-br from-purple/20 to-gold/10 gold-border font-bold text-sm">{tool.logo}</div><div><div className="font-bold text-lg">{tool.name}</div><div className="text-xs text-muted">{tool.vendor}</div></div></div>{tool.featured&&<span className="text-gold"><Sparkles size={17}/></span>}</div>
